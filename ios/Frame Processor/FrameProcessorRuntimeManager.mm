...
- (void) installFrameProcessorBindings {
  ...
  auto setVideoFrameProcessor = [self](jsi::Runtime& rnRuntime,
    ...
    RCTExecuteOnMainQueue([=]() {
      ...
      dispatch_async(CameraQueues.frameProcessorQueue, [=]() {
        ...
        view.videoFrameProcessorCallback = ^(Frame* frame) {
  ...
  jsiRuntime.global().setProperty(jsiRuntime, "setVideoFrameProcessor", jsi::Function::createFromHostFunction(jsiRuntime,
                                                                                                         jsi::PropNameID::forAscii(jsiRuntime, "setVideoFrameProcessor"),
                                                                                                         2,  // viewTag, frameProcessor
                                                                                                         setVideoFrameProcessor));

  auto setAudioFrameProcessor = [self](jsi::Runtime& rnRuntime,
                                    const jsi::Value& thisValue,
                                    const jsi::Value* arguments,
                                    size_t count) -> jsi::Value {
      self->workletRuntime = reanimated::extractWorkletRuntime(rnRuntime, arguments[2].asObject(rnRuntime));
      ...
      for (NSString* pluginKey in [FrameProcessorPluginRegistry frameProcessorPlugins]) {
        ...
        NSLog(@"FrameProcessorBindings: Installing Audio Frame Processor plugin \"%s\"...", pluginName);
        FrameProcessorPlugin callback = [[FrameProcessorPluginRegistry frameProcessorPlugins] valueForKey:pluginKey];
      ...
      RCTExecuteOnMainQueue([=]() {
        ...
        dispatch_async(CameraQueues.frameProcessorQueue, [=]() {
          ...
          view.audioFrameProcessorCallback = ^(Frame* frame) {
            auto workletRuntime = weakWorkletRuntime.lock();
            auto shareableWorklet = weakShareableWorklet.lock();
            if (workletRuntime == nullptr || shareableWorklet == nullptr) {
              return;
  ...
  jsiRuntime.global().setProperty(jsiRuntime, "unsetVideoFrameProcessor", jsi::Function::createFromHostFunction(jsiRuntime,
                                                                                                           jsi::PropNameID::forAscii(jsiRuntime, "unsetVideoFrameProcessor"),
                                                                                                           1,  // viewTag
    ...
    auto unsetAudioFrameProcessor = [](jsi::Runtime& runtime,
      ...
      if (!arguments[0].isNumber()) throw jsi::JSError(runtime, "Camera::unsetAudioFrameProcessor: First argument ('viewTag') must be a number!");
      auto viewTag = arguments[0].asNumber();
